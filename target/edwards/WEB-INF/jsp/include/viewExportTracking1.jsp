...
<%
	...
	//XML 데이터를 호출할 URL
	String url = "https://unipass.customs.go.kr:38010/ext/rest/expDclrNoPrExpFfmnBrkdQry/retrieveExpDclrNoPrExpFfmnBrkd?crkyCn=m250e106p072g093e050m090e0&blNo="+blNo+"";

	//서버에서리턴될 XML데이터의 엘리먼트 이름 배열
	String[] fieldNames ={"acptDt", "exppnConm", "loadDtyTmlm", "shpmAirptPortNm", "tkofDt", "mrn", "shpmPckGcnt", "csclPckUt", "shpmPckUt", "shpmCmplYn", "shpmWght", "dvdeWdrw", "expDclrNo", "csclWght", "csclPckGcnt"};
	...
	try {
		...
		NodeList items = doc.getElementsByTagName("expDclrNoPrExpFfmnBrkdBlNoQryRsltVo");
  ...
  <body>
	...
	%>
			  ...
			  <tr>
			    ...
			    <td class="left">분할회수</td>
			    <td><%=pub.get("dvdeWdrw") %></td>
			  </tr>
			  <tr>
		...
		%>
			  ...
			  <tr>
			    <td class="left">선적지</td>
			    <td><%=pub.get("shpmAirptPortNm") %></td>
			    <td class="left">출항일자</td>
	    <%
			if(pub.get("tkofDt")==null||pub.get("tkofDt")==""){
		%>
			    <td></td>
		<%
			}else{
		%>
				<td><%=pub.get("tkofDt").toString().substring(0,4) %>-<%=pub.get("tkofDt").toString().substring(4,6) %>-<%=pub.get("tkofDt").toString().substring(6,8) %></td>
		<%
			}
		%>
			  </tr>
