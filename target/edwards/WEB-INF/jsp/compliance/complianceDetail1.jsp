					      ...
					      <script>
						    ...
						    function process_wb(wb) {
					        	...
					        	for(var i = 0; i < aacc.length; i++){
						        	var url 	= "../apis/compliance/insertSikMaster",
							    		params  = aacc[i],
							    		type 	= "POST";

						        	params["saup"] 			= (aacc[i].사업자 != undefined) ? aacc[i].사업자 : '';
						        	params["bl"] 			= (aacc[i].비엘 != undefined) ? aacc[i].비엘 : '';
						        	params["inv"] 			= (aacc[i].인보이스 != undefined) ? aacc[i].인보이스 : '';
						        	params["iphang"] 		= (aacc[i].입항일 != undefined) ? aacc[i].입항일.replace('-','').replace('-','') : '';
						        	params["banip"] 		= (aacc[i].반입일 != undefined) ? aacc[i].반입일.replace('-','').replace('-','') : '';
						        	params["singo"] 		= (aacc[i].신고일 != undefined) ? aacc[i].신고일.replace('-','').replace('-','') : '';
						        	params["suri"] 			= (aacc[i].수리일 != undefined) ? aacc[i].수리일.replace('-','').replace('-','') : '';
						        	params["code"] 			= (aacc[i].제품코드 != undefined) ? aacc[i].제품코드 : '';
						        	params["eng"] 			= (aacc[i].영문명 != undefined) ? aacc[i].영문명 : '';
						        	params["kor"] 			= (aacc[i].한글명 != undefined) ? aacc[i].한글명 : '';
						        	params["hs"] 			= (aacc[i].세번 != undefined) ? aacc[i].세번.replace('-','').replace('.','') : '';
						        	params["won"] 			= (aacc[i].원산지 != undefined) ? aacc[i].원산지 : '';
						        	params["jaejo"] 		= (aacc[i].제조코드 != undefined) ? aacc[i].제조코드 : '';
						        	params["product"] 		= (aacc[i].제조사 != undefined) ? aacc[i].제조사 : '';
						        	params["suip"] 			= (aacc[i].수입자 != undefined) ? aacc[i].수입자 : '';
						        	params["sdate"] 		= (aacc[i].검역시작 != undefined) ? aacc[i].검역시작.replace('-','').replace('-','') : '';
						        	params["edate"] 		= (aacc[i].검역완료 != undefined) ? aacc[i].검역완료.replace('-','').replace('-','') : '';
						        	params["gubun"] 		= (aacc[i].구분 != undefined) ? aacc[i].구분 : '';
						        	params["jungmil"] 		= (aacc[i].정밀 != undefined) ? aacc[i].정밀 : '';
						        	params["label"] 		= (aacc[i].라벨 != undefined) ? aacc[i].라벨 : '';
						        	params["susulu"] 		= (aacc[i].수수료 != undefined) ? aacc[i].수수료 : '';
						        	params["silbi1"] 		= (aacc[i].실비1 != undefined) ? aacc[i].실비1 : '';
						        	params["silbi2"] 		= (aacc[i].실비2 != undefined) ? aacc[i].실비2 : '';
						        	params["jungyn"] 		= (aacc[i].정밀여부 != undefined) ? aacc[i].정밀여부 : '';
						        	params["billyn"] 		= (aacc[i].계산서여부 != undefined) ? aacc[i].계산서여부.replace('.','').replace('.','') : '';
						        	params["sunyn"] 		= (aacc[i].선별여부 != undefined) ? aacc[i].선별여부 : '';
						        	params["ani"] 			= (aacc[i].검역대상 != undefined) ? aacc[i].검역대상 : '';
						        	params["remark"] 		= (aacc[i].리마크 != undefined) ? aacc[i].리마크 : '';
						        	params["etc"] 			= (aacc[i].비고 != undefined) ? aacc[i].비고 : '';
