$(document).ready(function(){
	...
	var url 	= "../apis/customs/selectFieldManage",
		params 	= {
			"fieldMngKey" 	: $("#fieldMngKey").val()
		},
		type 	= "POST";

	sendAjax(url, params, type, function(d){
		console.log(d);
		$("#regDate").val(d[0].regDate);
		$("#Impo_segwan").val(d[0].segwan);
		$("#jisa").val(d[0].jisa);
		$("#team").val(d[0].team);
		$("#userNm").val(d[0].userNm);
		$("#gubun").val(d[0].gubun);
		$("#singoCode").val(d[0].singoCode);
		$("#singoNum").val(d[0].singoNum);
		$("#company").val(d[0].company);
		$("#gumYn").val(d[0].gumYn);
		$("#jangchi").val(d[0].jangchi);
		$("#gwanUser").val(d[0].gwanUser);
		$("#jubsu").val(d[0].jubsu);
...
var fn_saveAction = function(status){
	...
	try {
		if (status=='Modify'){
			var url 	= "../apis/customs/updateFieldManage",
				params 	= {
					"fieldMngKey"	: $('#fieldMngKey').val(),
