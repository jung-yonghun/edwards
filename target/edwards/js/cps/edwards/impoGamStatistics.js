function selectImpoGamList(callback){
	progress.show();
	var url 	= "../apis/edwards/selectImpoGamList",
		params 	= {
...
function selectImpoGamExecList(callback){
	progress.show();
	var url 	= "../apis/edwards/selectImpoGamExecList",
		params 	= {
			"OWN_GODS_NM" 	: $('#OWN_GODS_NM').val(),
			"IMPT_DECL_NO" 	: $('#IMPT_DECL_NO').val(),
			"FROM_DT"		: $('#FROM_DT').val(),
	...
	if(isEmpty($('#ID').val())){
		...
		$('#masterGrid').jqGrid({
            ...
            colModel: [
                ...
                {label:'수리일', name:'DECL_CMPL_DT', index:'DECL_CMPL_DT', width:80, align:'center'},
                ...
                {label:'란번호', name:'LAN', index:'LAN', width:50, align:'center'},
                {label:'HS코드', name:'HS_CD', index:'HS_CD', width:100, align:'center', formatter:linkHsCodeFormatter},
                {label:'감면구분', name:'semi', index:'semi', width:80, align:'center'},
                {label:'행번호', name:'HNG', index:'HNG', width:50, align:'center'},
                ...
                {label:'이행량', name:'EXPT_EXEC_QTY', index:'EXPT_EXEC_QTY', width:50, align:'right', formatter:'number', formatoptions:{decimalPlaces: 0, thousandsSeparator: ","}},
                {label:'잔량', name:'EXEM_RMID_QTY', index:'EXEM_RMID_QTY', width:50, align:'right', formatter:'number', formatoptions:{decimalPlaces: 0, thousandsSeparator: ","}},
        ...
        $('#masterGrid1').jqGrid({
            ...
            colModel: [
                ...
                {label:'화주', name:'OWN_GODS_NM', index:'OWN_GODS_NM', width:180},
                {label:'수입신고번호', name:'IMPT_DECL_NO1', index:'IMPT_DECL_NO1', width:120, align:'center'},
                {label:'BL번호', name:'BL_NO', index:'BL_NO', width:170, width:150, align:'center', formatter:linkBlNoFormatter},
                {label:'란번호', name:'LAN', index:'LAN', width:50, align:'center'},
                {label:'HS코드', name:'HS_CD', index:'HS_CD', width:100, align:'center', formatter:linkHsCodeFormatter},
                {label:'행번호', name:'HNG', index:'HNG', width:50, align:'center'},
                ...
                {label:'수입량', name:'QTY', index:'QTY', width:50, align:'right', formatter:'number', formatoptions:{decimalPlaces: 0, thousandsSeparator: ","}},
                {label:'단위', name:'QTY_UNIT', index:'QTY_UNIT', width:50, align:'center'},
                {label:'작업명', name:'JOB_NM', index:'JOB_NM', width:150},
                {label:'사업부', name:'edwards', index:'edwards', width:80, align:'center'},
                {label:'용도이행번호', name:'EXEM_EXEC_MNG_NO', index:'EXEM_EXEC_MNG_NO', width:100, align:'center'},
                {label:'용도이행일', name:'EXEC_DT1', index:'EXEC_DT1', width:80, align:'center'},
                {label:'Item코드', name:'ITEM_CD1', index:'ITEM_CD1', width:100, align:'center'},
                {label:'Item명', name:'ITEM_NM1', index:'ITEM_NM1', width:200},
                {label:'이행량', name:'QTY1', index:'QTY1', width:50, align:'right', formatter:'number', formatoptions:{decimalPlaces: 0, thousandsSeparator: ","}}
            ],
            height: 400,
            rowNum: "50",
            rowList: [10, 20, 30, 40, 50, 100],
		...
		$('#tabs').tabs({
		    onSelect : function(title, index){
		    	...
		    	if(check == 0 && status > 365){
					...
					if(hest == 0){
						var url 	= "../apis/edwards/selectImpoGamListCount",
							params 	= {
								"OWN_GODS_NM" 	: $('#OWN_GODS_NM').val(),
								"IMPT_DECL_NO" 	: $('#IMPT_DECL_NO').val(),
								"FROM_DT"		: $('#FROM_DT').val(),
						...
						var url 	= "../apis/edwards/selectImpoGamExecListCount",
							params 	= {
								"OWN_GODS_NM" 	: $('#OWN_GODS_NM').val(),
								"IMPT_DECL_NO" 	: $('#IMPT_DECL_NO').val(),
								"FROM_DT"		: $('#FROM_DT').val(),
						...
						sendAjax(url, params, type, function(d){
							if(parseInt(d[0].COUNT) > 50000){
								...
								selectImpoGamExecList(function (d) {
...
var fn_searchExcel = function(){
	var form = "<form action='../apis/edwards/downloadImGam1' method='post'>";
...
var fn_searchExcel1 = function(){
	var form = "<form action='../apis/edwards/downloadImGam2' method='post'>";
		form += "<input type='hidden' name='OWN_GODS_NM' 	value='"+ $('#OWN_GODS_NM').val() +"' />";
		form += "<input type='hidden' name='IMPT_DECL_NO' 	value='"+ $('#IMPT_DECL_NO').val() +"' />";
		form += "<input type='hidden' name='FROM_DT' 		value='"+ $('#FROM_DT').val() +"' />";
		form += "<input type='hidden' name='TO_DT' 			value='"+ $('#TO_DT').val() +"' />";
