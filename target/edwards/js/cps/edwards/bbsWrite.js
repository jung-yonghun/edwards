	...
	$(function(){
		...
		$('#masterGrid').datagrid({
			...
			columns			: [[
                ...
                {field:'FinishedDay',title:'등록일',width:40,align:'center',formatter:linkUnixFormatter}
	        ]],
...
var fn_bindData = function(NTAAKey){
	...
	sendAjax(url, params, type, function(d){
		...
		$("#tx_editor_form #Keyword").val(d[0].Keyword);
...
function fn_insertAction(){
	...
	params = {
    	"category"			: $('#frmCommon #category').val(),
    	"subject" 			: "",
    	"keyword" 			: "",
    	"contents" 			: "",
    	"noticesYn" 		: "N",
	...
	sendAjax(url, params, type, function(d){
		$('#tx_editor_form #NTAAKey').val(d[0].NTAAKey);
		$('#tx_editor_form #category').val($('#frmCommon #category').val());
	});
}
	...
	try{
		var url 	= "../apis/system/updateNTAA100",
			params 	= {
				...
				"keyword" 	: $("#tx_editor_form #Keyword").val(),
				"contents" 	: $("#tx_editor_form #Contents").val()
			},
			type 	= "POST";
		params["useYn"] = "Y";
