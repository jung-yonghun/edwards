	...
	$(function(){
		$('#masterGrid').datagrid({
			...
			columns			: [[
                ...
                {field:'startLocation',title:'Cargo location',width:100}
	        ]]
		});
		...
		if (status=='insert'){
			...
			sendAjax(url, params, type, function(d){
				...
				if(d.content.length > 0){
					if(confirm("같은 Invoice No가 존재합니다. 그래도 저장하시겠습니까?")){
				...
				for (var i = 0; i < rows.length; i++) {
			        var url = "../apis/customs/saveRequest",
					    params = {
				            ...
				            "startNote"				: "",
				            "startCompensationYn"	: rows[i].startCompensationYn,
				            "startLocation"			: rows[i].startLocation,
				            "useYn"					: "Y",
...
var fn_sampleAction = function(){
    document.location.href="../images/common/expoRequestSample.xlsx";
}