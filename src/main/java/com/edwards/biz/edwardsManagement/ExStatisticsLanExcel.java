...
public class ExStatisticsLanExcel implements ResultHandler<Map<String,Object>> {
	...
	public ExStatisticsLanExcel(HttpServletResponse response){
	...
	public void handleResult(ResultContext<? extends Map<String, Object>> result){
		try {
			if(!isStarted){
        		...
        		response.setHeader("Content-Disposition","attachment;filename=\""+URLEncoder.encode("수출란기준실적자료.xlsx", "UTF-8")+"\"");
        		isStarted = true;
        	...
        	cell = row.createCell(19);
        	cell.setCellValue(Double.parseDouble(dataRow.get("ExlanFobUsd").toString())==0 ? 0 : Double.parseDouble(dataRow.get("ExlanFobUsd").toString()));
        	cell.setCellStyle(style20);
        	cell = row.createCell(20);
