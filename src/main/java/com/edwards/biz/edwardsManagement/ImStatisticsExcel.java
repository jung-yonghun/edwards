...
public class ImStatisticsExcel implements ResultHandler<Map<String,Object>> {
	...
	public ImStatisticsExcel(HttpServletResponse response){
		...
		sheet.setColumnWidth(27, 3000);	//결제금액
	}
	...
	public void handleResult(ResultContext<? extends Map<String, Object>> result){
		try {
			if(!isStarted){
        		...
        		response.setHeader("Content-Disposition","attachment;filename=\""+URLEncoder.encode("수입공통기준실적자료.xlsx", "UTF-8")+"\"");
        		isStarted = true;
        		...
        		cell.setCellValue("ShippingCharge");
        		cell.setCellStyle(style);
        	}
