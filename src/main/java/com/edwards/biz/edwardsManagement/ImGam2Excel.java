...
public class ImGam2Excel implements ResultHandler<Map<String,Object>> {
	...
	public ImGam2Excel(HttpServletResponse response){
		this.response = response;
		workbook = new SXSSFWorkbook(100);
		sheet = workbook.createSheet("감면수입물품 이행처리내역");
		...
		sheet.setColumnWidth(5, 1500);	//란번호
		sheet.setColumnWidth(6, 4000);	//HS코드
		sheet.setColumnWidth(7, 1500);	//행번호
		sheet.setColumnWidth(8, 3000);	//아이템코드
		sheet.setColumnWidth(9, 7000);	//아이템명
		sheet.setColumnWidth(10, 3000);	//인보이스번호
		sheet.setColumnWidth(11, 3000);	//수입량
		sheet.setColumnWidth(12, 1500);	//단위
		sheet.setColumnWidth(13, 5000);	//작업명
		sheet.setColumnWidth(14, 3000);	//사업부
		sheet.setColumnWidth(15, 4000);	//용도이행번호
		sheet.setColumnWidth(16, 3000);	//용도이행일
		sheet.setColumnWidth(17, 3000);	//아이템코드
		sheet.setColumnWidth(18, 7000);	//아이템명
		sheet.setColumnWidth(19, 1500);	//이행량
	}
	...
	public void handleResult(ResultContext<? extends Map<String, Object>> result){
		try {
			if(!isStarted){
        		...
        		response.setHeader("Content-Disposition","attachment;filename=\""+URLEncoder.encode("감면수입물품이행처리내역.xlsx", "UTF-8")+"\"");
        		isStarted = true;
        		...
        		cell.setCellValue("작업명");
        		cell.setCellStyle(style);
        		cell = row.createCell(14);
        		cell.setCellValue("사업부");
        		cell.setCellStyle(style);
        		cell = row.createCell(15);
        		cell.setCellValue("용도이행번호");
        		cell.setCellStyle(style);
        		cell = row.createCell(16);
        		cell.setCellValue("용도이행일");
        		cell.setCellStyle(style);
        	...
        	cell = row.createCell(3);
        	cell.setCellValue(dataRow.get("IMPT_DECL_NO1")==null ? "" : dataRow.get("IMPT_DECL_NO1").toString());
        	cell.setCellStyle(style);
        	cell = row.createCell(4);
        	...
        	cell.setCellStyle(style);
        	cell = row.createCell(13);
        	cell.setCellValue(dataRow.get("JOB_NM")==null ? "" : dataRow.get("JOB_NM").toString());
        	cell.setCellStyle(style1);
        	cell = row.createCell(14);
        	cell.setCellValue(dataRow.get("edwards")==null ? "" : dataRow.get("edwards").toString());
        	cell.setCellStyle(style);
        	cell = row.createCell(15);
        	cell.setCellValue(dataRow.get("EXEM_EXEC_MNG_NO")==null ? "" : dataRow.get("EXEM_EXEC_MNG_NO").toString());
        	cell.setCellStyle(style);
        	cell = row.createCell(16);
        	cell.setCellValue(dataRow.get("EXEC_DT1")==null ? "" : dataRow.get("EXEC_DT1").toString());
        	cell.setCellStyle(style);
        	cell = row.createCell(17);
        	cell.setCellValue(dataRow.get("ITEM_CD1")==null ? "" : dataRow.get("ITEM_CD1").toString());
        	cell.setCellStyle(style);
        	cell = row.createCell(18);
        	cell.setCellValue(dataRow.get("ITEM_NM1")==null ? "" : dataRow.get("ITEM_NM1").toString());
        	cell.setCellStyle(style1);
        	cell = row.createCell(19);
        	cell.setCellValue(Double.parseDouble(dataRow.get("QTY1").toString())==0 ? 0 : Double.parseDouble(dataRow.get("QTY1").toString()));
        	cell.setCellStyle(style20);
		} catch (Exception e) {
