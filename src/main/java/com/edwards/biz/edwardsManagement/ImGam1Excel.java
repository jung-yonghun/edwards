...
public class ImGam1Excel implements ResultHandler<Map<String,Object>> {
	...
	public ImGam1Excel(HttpServletResponse response){
		this.response = response;
		workbook = new SXSSFWorkbook(100);
		sheet = workbook.createSheet("감면수입물품내역");
		...
		sheet.setColumnWidth(7, 3000);	//감면구분
		sheet.setColumnWidth(8, 1500);	//행번호
		sheet.setColumnWidth(9, 3000);	//아이템코드
		sheet.setColumnWidth(10, 7000);	//아이템명
		sheet.setColumnWidth(11, 2000);	//수입량
		sheet.setColumnWidth(12, 2000);	//이행량
		sheet.setColumnWidth(13, 2000);	//잔량
		sheet.setColumnWidth(14, 1500);	//단위
	}
	...
	public void handleResult(ResultContext<? extends Map<String, Object>> result){
		try {
			if(!isStarted){
        		...
        		response.setHeader("Content-Disposition","attachment;filename=\""+URLEncoder.encode("감면수입물품내역.xlsx", "UTF-8")+"\"");
        		isStarted = true;
        	...
        	cell.setCellStyle(style);
        	cell = row.createCell(7);
        	cell.setCellValue(dataRow.get("semi")==null ? "" : dataRow.get("semi").toString());
        	cell.setCellStyle(style);
        	cell = row.createCell(8);
        	...
        	cell = row.createCell(12);
        	cell.setCellValue(Double.parseDouble(dataRow.get("EXPT_EXEC_QTY").toString())==0 ? 0 : Double.parseDouble(dataRow.get("EXPT_EXEC_QTY").toString()));
        	cell.setCellStyle(style20);
        	cell = row.createCell(13);
        	cell.setCellValue(Double.parseDouble(dataRow.get("EXEM_RMID_QTY").toString())==0 ? 0 : Double.parseDouble(dataRow.get("EXEM_RMID_QTY").toString()));
        	cell.setCellStyle(style20);
        	cell = row.createCell(14);
