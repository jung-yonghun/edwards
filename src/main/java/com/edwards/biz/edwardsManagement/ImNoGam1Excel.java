...
public class ImNoGam1Excel implements ResultHandler<Map<String,Object>> {
	...
	public ImNoGam1Excel(HttpServletResponse response){
		this.response = response;
		workbook = new SXSSFWorkbook(100);
		sheet = workbook.createSheet("비감면수입물품내역");
		...
		sheet.setColumnWidth(12, 2000);	//수입량
		sheet.setColumnWidth(13, 2000);	//원상태수출량
		sheet.setColumnWidth(14, 2000);	//환급량
		sheet.setColumnWidth(15, 2000);	//잔량
		sheet.setColumnWidth(16, 1500);	//단위
	}
	...
	public void handleResult(ResultContext<? extends Map<String, Object>> result){
		try {
			if(!isStarted){
        		...
        		response.setHeader("Content-Disposition","attachment;filename=\""+URLEncoder.encode("비감면수입물품내역.xlsx", "UTF-8")+"\"");
        		isStarted = true;
        		...
        		cell.setCellValue("원상태수출량");
        		cell.setCellStyle(style);
        		cell = row.createCell(14);
        		cell.setCellValue("환급량");
        		cell.setCellStyle(style);
        	...
        	cell.setCellStyle(style20);
        	cell = row.createCell(14);
        	cell.setCellValue(Double.parseDouble(dataRow.get("REFUND_QTY").toString())==0 ? 0 : Double.parseDouble(dataRow.get("REFUND_QTY").toString()));
        	cell.setCellStyle(style20);
        	cell = row.createCell(15);
        	cell.setCellValue(Double.parseDouble(dataRow.get("RMID_QTY").toString())==0 ? 0 : Double.parseDouble(dataRow.get("RMID_QTY").toString()));
        	cell.setCellStyle(style20);
        	cell = row.createCell(16);
