...
(function ($) {
    if($.fn.ajaxForm == undefined) {
    	...
    	$.getScript("../js/lib/jquery.form/jquery.form.js");
    }
    var feature = {};
    ...
    $.fn.uploadFile = function (options) {
        ...
        this.hide();
        this.errorLog = $("<marquee width='300' height='20' behavior='scroll' scrolldelay='1000' direction='up'><div id='mymarquee'></div></marquee>"); //Writing errors
...
//                }
                if(!isFileTypeAllowed(obj, s, files[i].name)) {
                    if(s.showError) $("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.extErrorStr + "</div>").appendTo(obj.errorLog);
                    continue;
                }
...
//                }
                if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if(s.showError) $("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> " + s.sizeErrorStr + "</div>").appendTo(
                        obj.errorLog);
                    continue;
                }
                ...
                if(s.showStatusAfterSuccess) {
                	$("<div class='" + s.errorClass + "'><b>" + files[i].name + "</b> 전송 성공</div>").appendTo(obj.errorLog);
                	continue;
                }
                obj.selectedFiles++;
        ...
        function createProgressDiv(obj, s) {
            this.statusbar = $("<div class='ajax-file-upload-statusbar'></div>").width(s.statusBarWidth).hide();
            ...
            if(s.showQueueDiv)
                $("#" + s.showQueueDiv).append(this.statusbar).hide();
            else
