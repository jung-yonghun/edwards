...
var fn_saveAction = function(){
	...
	if(document.frm1.ExEmNo.value=="" && document.frm1.SERIAL_NO.value==""){
		alert("면제번호나 Serial No를 입력하세요.");
        return;
	}

	if(document.frm1.EXPT_QTY.value=="0" || parseInt(document.frm1.EXPT_QTY.value) < 0){
		alert("수출량이 0이하 일수는 없습니다.");
        return;
	}

	var EXPT_QTY = 0;
	if(document.frm1.EXPT_QTY.value==""){
		EXPT_QTY = 0;
	}else{
		EXPT_QTY = parseInt(document.frm1.EXPT_QTY.value);
	}

	if(parseInt(document.frm1.IMPT_QTY.value) < EXPT_QTY){
	...
	if(EXPT_QTY==0){
		...
		// 한개만 있고 자신것 수정이면
		if(parseInt(document.frm1.IMPT_QTY.value)-parseInt(document.frm1.RMID_QTY.value)-EXPT_QTY==0){
			var url 	= "../apis/edwards/saveReExpo",
			    params 	= $("#frm1").serializeObject(),
			    type 	= "POST";

		    params["checkQty"] = parseInt(document.frm1.IMPT_QTY.value) - EXPT_QTY;
		    params["EXPT_INVNO"] = document.frm1.INV_NO.value;
		    params["Expt_decl_no"] = document.frm1.EXPT_DECL_NO.value;
		    params["Expt_cmpl_dt"] = document.frm1.EXPT_CMPL_DT.value;
		    params["Expt_lan"] = document.frm1.EXPT_LAN.value;
...
//				if(parseInt(d[0].ExCount) > 0){
					SumExQty = parseInt(document.frm1.RMID_QTY.value) + parseInt(document.frm1.EXPT_QTYORI.value) - EXPT_QTY;
