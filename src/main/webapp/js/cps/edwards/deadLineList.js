...
function selectCommonCode7(){
	var url 	= "../apis/edwards/selectCommonCode",
		params 	= {"Mcd" : "EXP_DCLR_WTHD_RCD"},
		type 	= "POST";

	sendAjax(url, params, type, function(d){
        $('#detailGrid64').datagrid('loadData', d);
	});
}
			...
			$('#masterGrid3').datagrid({
		        ...
		        onLoadSuccess	: function(rowIndex, rowData){
		        	...
		        	for(var i=0; i<rowIndex.rows.length; i++){
		            	if(aaa == rowIndex.rows[i].BL_NO && bbb == rowIndex.rows[i].IMPT_DECL_NO && ccc == rowIndex.rows[i].IMPT_LAN && ddd == rowIndex.rows[i].IMPT_HNG && eee == rowIndex.rows[i].SoNo && fff == rowIndex.rows[i].EndUserName && ggg == rowIndex.rows[i].ExEmNo && hhh == rowIndex.rows[i].SERIAL_NO && iii == rowIndex.rows[i].PROD_CD && jjj == rowIndex.rows[i].PROD_NM){
		            		k++;
...
//			});
			...
			$('#detailGrid64').datagrid('enableFilter',[]);
			$('#detailGrid64').datagrid('getPager').pagination({showPageList:false,showRefresh:false});
			},10);
...
var fn_codeExcel7 = function(){
	exportCsv("../apis/edwards/selectCommonCode", {"Mcd" : "EXP_DCLR_WTHD_RCD"}, $('#excelGrid51'),"CommonCode07");
};
	...
	if(confirm("[수정] 하시겠습니까?")){
		if(row[0].EXPT_ORDR_MNG_NO==null || row[0].EXPT_ORDR_MNG_NO == ""){
			if(row[0].IMPT_ORDR_MNG_NO==null || row[0].IMPT_ORDR_MNG_NO == ""){
				...
				openWindowWithPost("./ReImportUpdatePop.cps", "width=600, height=340, scrollbars=no, location=no, menubar=no", "ReImportUpdatePop", {
...
var fn_updateAction1 = function(){
	...
	if(confirm("[수정] 하시겠습니까?")){
		console.log(row[0].EXPT_ORDR_MNG_NO);
		if(row[0].IMPT_ORDR_MNG_NO==null || row[0].IMPT_ORDR_MNG_NO == ""){
			if(row[0].EXPT_ORDR_MNG_NO==null || row[0].EXPT_ORDR_MNG_NO == ""){
				...
				openWindowWithPost("./ReExportUpdatePop.cps", "width=600, height=420, scrollbars=no, location=no, menubar=no", "ReExportUpdatePop", {
...
var fn_addAction = function(){
	...
	if(confirm("[추가등록] 하시겠습니까?")){
		if(parseInt(row[0].RMID_QTY)==0){
			alert("추가등록할 잔량이 부족합니다.");
		}else if(parseInt(row[0].IMPT_QTY)-parseInt(row[0].RMID_QTY)==0){
			alert("잔량차감이 있는 경우에만 추가등록이 가능합니다.");
		}else{
			openWindowWithPost("./ReExportInsertPop.cps", "width=600, height=420, scrollbars=no, location=no, menubar=no", "ReExportUpdatePop", {
				"KEY_ED_REEXPT_MASTER" 	: row[0].KEY_ED_REEXPT_MASTER
			});
		}
	}
