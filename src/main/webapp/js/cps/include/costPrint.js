function selectCostList(){
    ...
    sendAjax(url, params, type, function(d){
        if (d.length != 1){
            $('#TITLE_NM').html("자금정산서 ("+$('#WORK_NM').val()+")");
        ...
        $('#ADJ_MEM_NM').html(d[0].ADJ_DOC_ADJ_MEM_NM);
        ...
        $('#PRE_JAN_EK').html($.number(d[0].ADJ_DOC_PRE_JAN_EK, 0));
        $('#TAKE_EK').html($.number(d[0].TAKE_EK, 0));
        $('#IN_EK').html($.number(d[0].IN_EK, 0));
        $('#IN_EK_NOTE').html(d[0].IN_EK_NOTE);
        $('#SEND_JAN_EK').html($.number(d[0].SEND_JAN_EK, 0));
        $('#SEND_JAN_EK_NOTE').html(d[0].SEND_JAN_EK_NOTE);
